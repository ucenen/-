<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button type="success" plain size="mini" @click="handleCopyClick"
          >复制至</el-button
        >
      </el-col>
    ...
    >
      ...
      />
      <el-table-column label="部门" v-if="columns[1].visible" align="center">
        <template slot-scope="scope">
          <dict-tag
            :options="dict.type.sys_dept_group"
            :value="scope.row.deptCode"
          />
        </template>
      </el-table-column>
      <el-table-column
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="submitFormCopy" :loading="saving"
          >确 定</el-button
        >
        <el-button @click="cancelFormCopy" :loading="saving">取 消</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      getCommissionMainList(this.queryParams)
        .then((response) => {
          this.commissionMainList = response.rows;
    ...
    handleUpdate(row) {
      ...
      this.$router.push({
        path: "/store/base/commission/commissionAdd",
        query: {
          id: String(row.id),
        },
      });
    ...
    handleDelete() {},
    handleAdd() {
      this.$router.push("/store/base/commission/commissionAdd");
    },
    cancelFormCopy() {
